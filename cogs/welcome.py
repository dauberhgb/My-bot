...
class Welcome(commands.Cog):
    ...
    # ==========================================
    async def generate_welcome_card(self, member, bg_url=None, lang="ar", frame_key=None, guild_id=None):
      try:
        width, height = 1536, 1024
        # إنشاء خلفية شفافة بالكامل
        base = Image.new("RGBA", (width, height), (0, 0, 0, 0))
          
        settings = db.get_settings(guild_id) if guild_id else {}
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(bg_url) as resp:
                        if resp.status == 200:
                            ...
                            custom_bg = Image.open(io.BytesIO(bg_data)).convert("RGBA")
    ...
    async def on_member_join(self, member):
      ...
      if welcome_channel_id and str(welcome_channel_id).isdigit():
        ...
        if channel:
          ...
          card_file = await self.generate_welcome_card(member, bg_url, lang=lang, frame_key=frame_key, guild_id=member.guild.id)
          if card_file:
